...
class CelebADataset(Dataset):
    ...
    def __init__(self, mode, train_filepath, train_csvpath):
        #self.train_data = read_image(train_filepath)
        #self.test_data = read_image(test_filepath)     
        self.mode = mode

        if self.mode == 'GAN':
            self.images = read_image_gan([train_filepath], True)

        if self.mode == 'ACGAN':
            self.images = read_image_gan([train_filepath], False)
            self.train_attr = pd.read_csv(train_csvpath)['Smiling']
            #self.attr = torch.FloatTensor(self.attr)
            self.attr = torch.FloatTensor(self.train_attr).view(-1, 1, 1, 1)
